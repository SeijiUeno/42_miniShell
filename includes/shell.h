...
/*   Updated: 2024/12/14 16:02:53 by sueno-te         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_command					*build_pipeline_ast(t_minishell *minishell,
								t_command *root_cmd);
t_command					*create_command_node(t_token *tokens,
								t_minishell *minishell);
...
char						*expand_word(const char *input_str, int *index);
char						*expand_env(const char *var_name, char **envp);
char						*expand_parameter(const char *input_str, int *index,
								t_minishell *minishell);
char						*join_word(char *word, char *new_word);
char						*expand_until_char(const char *input_str,
								int *index, char stop_char);
>>>>>>> origin/norm-expansor
...
void						exec_child_process(t_minishell *minishell,
								t_command *temp_tree, int is_left);
void						exec_run_child_command(t_minishell *m,
								t_command *cmd, int *pipes, int ctx[2]);
void						exec_wait_for_children(t_minishell *minishell,
								pid_t *pids, int count);
// pipes
void						pipe_close_all(int *pipes, int total_fds);
int							pipe_create(int *pipes, int command_count);
void						pipe_run_pipeline(t_minishell *m, t_command **cmds,
								int cmd_count);
// paths
char						*define_full_path(char *cmd, char **path);
char						*ft_strjoin_three(const char *s1, const char *s2,
								const char *s3);
int							is_valid_command(char **full_path, const char *cmd,
								char **env_path);
