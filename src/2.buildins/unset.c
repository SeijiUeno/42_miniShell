...
/*   Updated: 2024/12/14 18:28:19 by emorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	copy_filtered_envp(char **envp, char **new_env, const char *key)
{
	int	i;
	int	j;
...
static int	unset_env(const char *key, t_minishell *minishell)
{
	...
	if (!new_env)
		return (EXIT_FAILURE);
	copy_filtered_envp(minishell->envp, new_env, key);
	free(minishell->envp);
