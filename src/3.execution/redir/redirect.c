...
/*   Updated: 2024/12/14 19:49:09 by emorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static int	process_single_redirection(t_token **token_list, t_token **redirs,
		t_token *redir_op, t_minishell *minishell)
{
...
t_token	*ft_generate_redirs(t_token **token_list, t_minishell *minishell)
{
	...
	while (current && current->type != PIPE)
	{
		...
		{
			if (process_single_redirection(token_list, &redirs, current,
					minishell) < 0)
				break ;
			current = *token_list;
		}
