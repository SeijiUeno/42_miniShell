...
/*   Updated: 2024/12/14 19:23:00 by emorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_child_fds(t_minishell *m, t_command *cmd, int *pipes, int ctx[2])
{
	...
	if (child_index > 0 && dup2(pipes[(child_index - 1) * 2], STDIN_FILENO) < 0)
		util_free_child(m);
	if (child_index < cmd_count - 1 && dup2(pipes[child_index * 2 + 1],
			STDOUT_FILENO) < 0)
		util_free_child(m);
...
void	exec_run_child_command(t_minishell *m, t_command *cmd, int *pipes,
		int ctx[2])
{
	char	*cmd_path;
