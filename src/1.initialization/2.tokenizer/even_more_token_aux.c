...
/*   Updated: 2024/12/15 05:45:33 by sueno-te         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	process_symbol(char *input, int *index, t_token **tokens,
		t_token **current)
{
	t_token_data	data;
...
void	process_word(char *input, int *index, t_token **tokens,
		t_token **current)
{
	int				start;
	t_token_data	data;
		...
		&& !ft_strchr(SYMBOLS, input[*index]))
	{
		...
		else if (ft_strchr(SYMBOLS, input[*index]) && !ft_strchr(QUOTES,
				input[start]))
			break ;
		else
			(*index)++;
