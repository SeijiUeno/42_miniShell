...
/*   Updated: 2024/12/14 18:59:24 by emorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*extract_var_name(const char *input_str, int *index)
{
	...
	start_index = *index;
	var_len = 0;
	while (input_str[*index] && (isalnum((unsigned char)input_str[*index])
			|| input_str[*index] == '_'))
	{
		(*index)++;
		var_len++;
...
static char	*expand_special_parameter(const char *input_str, int *index,
...
static char	*get_variable_value(const char *input_str, int *index,
		t_minishell *minishell)
{
	char	*var_name;
	char	*expanded_value;
		...
		t_minishell *minishell)
{
	...
	if (!input_str[*index] || (!isalnum((unsigned char)input_str[*index])
